  ...
  return(
    <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 12px",marginBottom:8,position:"relative",boxShadow:T.shadow}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
        <div style={{width:30,height:30,borderRadius:6,background:extBg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:9,fontWeight:800,color:extColor}}>{ext.slice(0,4)}</div>
        <div style={{flex:1,minWidth:0}}>
          <p style={{margin:0,fontSize:12,color:T.text,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name}</p>
          <div style={{marginTop:4}}>
            {f.assignedCode
              ?<span onClick={()=>setOpen(o=>!o)} style={{cursor:"pointer",padding:"2px 8px",borderRadius:99,background:"#d1fae5",color:"#065f46",fontSize:11,fontWeight:700,border:"1px solid #a7f3d0"}}>↔ {f.assignedCode}</span>
              :<span onClick={()=>setOpen(o=>!o)} style={{cursor:"pointer",padding:"2px 8px",borderRadius:99,background:T.primaryBg,color:T.primary,fontSize:11,border:`1px solid ${T.borderMid}`}}>+ กำหนดรหัส</span>}
          </div>
        </div>
        <button onClick={onRemove} style={{background:"none",border:"none",color:T.textMuted,cursor:"pointer",fontSize:16,lineHeight:1,padding:2}}>×</button>
      </div>
      {open&&(
        <div style={{position:"absolute",left:0,right:0,top:"100%",zIndex:200,background:T.surface,border:`1px solid ${T.borderMid}`,borderRadius:10,boxShadow:T.shadowMd,maxHeight:220,overflowY:"auto",marginTop:4}}>
          <div style={{padding:"6px 12px",fontSize:11,color:T.textMuted,borderBottom:`1px solid ${T.border}`,fontWeight:600}}>เลือกเกณฑ์</div>
          {PA_SECTIONS.map(sec=>(
            <div key={sec.id}>
              <div style={{padding:"4px 12px",fontSize:10,color:sec.accent,fontWeight:800,background:`${sec.accent}10`}}>{sec.short}</div>
              {sec.items.map(item=>(
                <div key={item.code} onClick={()=>{onAssign(item.code);setOpen(false);}} style={{padding:"5px 14px",fontSize:11,color:T.text,cursor:"pointer"}} onMouseOver={e=>e.currentTarget.style.background=T.primaryBg} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"88px 1fr",gap:10}}>
        <div style={{paddingTop:2}}>
          <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
            ...
            <span style={{fontFamily:"monospace",fontWeight:800,fontSize:12,color:T.primaryDark}}>{item.code}</span>
          </div>
          <span style={{display:"inline-block",padding:"2px 7px",borderRadius:6,background:`${T.primary}15`,color:T.primary,fontSize:10,fontWeight:700}}>×{item.w}</span>
          {filesList.length>0&&(<div style={{marginTop:5}}>{filesList.map((f,i)=>(<div key={i} style={{fontSize:9,color:T.aiGreen,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:80}}>📎 {f.name.slice(0,14)}…</div>))}</div>)}
        </div>
        <div>
          <p style={{margin:"0 0 4px",fontSize:12,color:T.textMid,lineHeight:1.5}}>
            {item.desc}
            <span style={{cursor:"pointer",marginLeft:6,color:T.accent,fontSize:10,fontWeight:700}} onClick={()=>setShowHint(h=>!h)}>{showHint?"▲ซ่อน":"ℹ เกณฑ์"}</span>
          </p>
          {showHint&&(<p style={{margin:"0 0 5px",fontSize:11,color:"#64748b",background:"#f1f5f9",padding:"5px 8px",borderRadius:6,border:"1px solid #e2e8f0",lineHeight:1.5}}>{item.hint}</p>)}
          <textarea value={d.activity} onChange={e=>onUpdate(item.code,"activity",e.target.value)} placeholder="อธิบายกิจกรรม/ผลการปฏิบัติงาน (ภาษาไทย)..." rows={Math.max(2,(d.activity||"").split("\n").length+(d.activity?0:1))} style={{width:"100%",background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:7,color:T.text,fontSize:12,padding:"7px 10px",resize:"vertical",fontFamily:"'Sarabun',sans-serif",lineHeight:1.6,boxSizing:"border-box",outline:"none"}} onFocus={e=>e.target.style.borderColor=T.primaryLight} onBlur={e=>e.target.style.borderColor=T.border}/>
        </div>
      </div>
      <div style={{paddingLeft:10,display:"flex",flexDirection:"column",alignItems:"center",gap:7,paddingTop:2}}>
        <span style={{fontSize:10,color:T.textMuted,fontWeight:600}}>คะแนน</span>
        <span style={{fontSize:9,color:T.textMuted}}>(0–10)</span>
        <input type="number" min={0} max={10} step={0.5} value={d.score} onChange={e=>onUpdate(item.code,"score",e.target.value)} style={{width:56,background:T.surface,border:`2px solid ${d.score?T.primary:T.border}`,borderRadius:8,color:T.primaryDark,fontSize:18,fontWeight:700,textAlign:"center",padding:"5px 4px",outline:"none"}}/>
        <button onClick={()=>onExtract(item)} disabled={isExtracting||filesList.length===0} title={filesList.length===0?"แนบไฟล์ก่อน":"AI สกัดข้อมูลจากไฟล์"} style={{width:64,padding:"6px 0",borderRadius:7,border:"none",background:filesList.length===0?"#e2e8f0":isExtracting?T.borderMid:`linear-gradient(135deg,${T.primary},${T.accent})`,color:filesList.length===0?T.textMuted:"white",fontSize:11,cursor:filesList.length===0?"not-allowed":"pointer",fontWeight:600}}>
          {isExtracting?"⏳":"⚡ AI"}
        </button>
        <span style={{fontSize:10,color:dotColor,fontWeight:700}}>{status==="ai-filled"?"🤖":d.activity?"✓":"—"}</span>
      </div>
    </div>
  );
}
  ...
  return(
    <div style={{border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden",marginBottom:10,boxShadow:T.shadow}}>
      <div onClick={onToggle} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 20px",cursor:"pointer",background:isOpen?`${sec.accent}10`:T.surface,borderBottom:isOpen?`1px solid ${T.border}`:"none",userSelect:"none"}}>
        ...
        <div style={{flex:1}}><span style={{fontWeight:700,fontSize:13.5,color:T.text}}>{sec.title}</span></div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:12,color:T.textMuted,minWidth:48,textAlign:"right"}}>{filled}/{sec.items.length}</span>
          <span style={{color:sec.accent,fontSize:14,transform:isOpen?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s",fontWeight:700}}>▼</span>
        </div>
      </div>
  ...
  return(
    <div style={{fontFamily:"'Sarabun','Noto Sans Thai',sans-serif",background:T.bg,minHeight:"100vh",color:T.text}}>
      ...
      <header style={{background:T.surface,borderBottom:`2px solid ${T.borderMid}`,padding:"0 24px",position:"sticky",top:0,zIndex:50,boxShadow:T.shadow}}>
        <div style={{maxWidth:1400,margin:"0 auto",display:"flex",alignItems:"center",gap:16,height:60}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <h1 style={{margin:0,fontSize:17,fontWeight:800,color:T.primaryDark,lineHeight:1.2}}>PA Assistant</h1>
              <p style={{margin:0,fontSize:10,color:T.textMuted}}>คณะศิลปศาสตร์ มหาวิทยาลัยมหิดล</p>
            </div>
          </div>
          <div style={{flex:1,display:"flex",alignItems:"center",gap:8,padding:"0 16px"}}>
            ...
            <span style={{fontSize:12,color:T.textMid,fontWeight:600}}><span style={{color:T.primary,fontWeight:800}}>{filled}</span>/{TOTAL} ({pct}%)</span>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={()=>setShowInfo(h=>!h)} style={{padding:"7px 14px",borderRadius:8,border:`1.5px solid ${T.borderMid}`,background:showInfo?T.primaryBg:T.surface,color:T.primaryDark,fontSize:12,fontWeight:600,cursor:"pointer"}}>👤 ข้อมูลส่วนตัว</button>
            <button onClick={extractAll} style={{padding:"7px 16px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${T.primary},${T.accent})`,color:"white",fontSize:12,fontWeight:700,cursor:"pointer",boxShadow:T.shadow}}>⚡ AI สกัดทั้งหมด</button>
            <button onClick={handleExport} style={{padding:"7px 16px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#059669,#047857)",color:"white",fontSize:12,fontWeight:700,cursor:"pointer",boxShadow:T.shadow}}>📥 Export Word</button>
          </div>
        </div>
      </header>
        ...
        <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"16px 24px"}}>
          <div style={{maxWidth:1400,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
            {[["name","ชื่อ-สกุล"],["position","ตำแหน่ง"],["department","สังกัดหลักสูตร"],["faculty","คณะ"],["supervisor","ผู้บังคับบัญชา"],["period","รอบการประเมิน"]].map(([k,label])=>(
              <div key={k}>
                <label style={{display:"block",fontSize:11,color:T.textMuted,marginBottom:4,fontWeight:700}}>{label}</label>
                <input value={info[k]} onChange={e=>setInfo(p=>({...p,[k]:e.target.value}))} style={{width:"100%",background:T.bg,border:`1.5px solid ${T.border}`,borderRadius:7,color:T.text,fontSize:13,padding:"7px 10px",boxSizing:"border-box",outline:"none",fontFamily:"'Sarabun',sans-serif"}} onFocus={e=>e.target.style.borderColor=T.primary} onBlur={e=>e.target.style.borderColor=T.border}/>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* BODY */}
      <div style={{maxWidth:1400,margin:"0 auto",display:"grid",gridTemplateColumns:"290px 1fr",gap:0,minHeight:"calc(100vh - 60px)"}}>

        {/* FILE PANEL */}
        <aside style={{background:T.surface,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",minHeight:0,position:"sticky",top:60,height:"calc(100vh - 60px)",overflow:"hidden"}}>
          <div style={{padding:"14px 16px",borderBottom:`1px solid ${T.border}`,background:`${T.primary}08`}}>
            <p style={{margin:0,fontSize:13,fontWeight:800,color:T.primaryDark}}>📁 ไฟล์หลักฐาน</p>
            <p style={{margin:"2px 0 0",fontSize:11,color:T.textMuted}}>{files.length} ไฟล์ | ลากมาวางหรือคลิกเลือก</p>
          </div>
          <div onDrop={handleDrop} onDragOver={e=>{e.preventDefault();setIsDragging(true);}} onDragLeave={()=>setIsDragging(false)} onClick={()=>fileInputRef.current?.click()} style={{margin:"12px 12px 0",border:`2px dashed ${isDragging?T.primaryLight:T.borderMid}`,borderRadius:12,padding:"18px 12px",textAlign:"center",cursor:"pointer",background:isDragging?`${T.primary}08`:T.bg,transition:"all 0.2s"}}>
            ...
            <p style={{margin:0,fontSize:12,color:T.textMid,fontWeight:600}}>ลากไฟล์มาวางที่นี่</p>
            <p style={{margin:"4px 0 0",fontSize:10,color:T.textMuted}}>PDF · DOCX · JPG · PNG</p>
          ...
          <div style={{margin:"8px 12px 0",padding:"8px 10px",background:"#fffbeb",borderRadius:8,border:"1px solid #fde68a"}}>
            <p style={{margin:0,fontSize:10,color:"#92400e",lineHeight:1.5}}>💡 ชื่อไฟล์ขึ้นต้นด้วยรหัส เช่น <strong>3_4_จดหมาย...</strong> → จับคู่กับ 3.4 อัตโนมัติ</p>
          </div>
          <div style={{flex:1,overflowY:"auto",padding:"8px 12px"}}>
            {files.length===0?(<div style={{textAlign:"center",padding:"30px 12px",color:T.textMuted,fontSize:12,lineHeight:1.8}}>ยังไม่มีไฟล์<br/>อัปโหลดไฟล์หลักฐานเพื่อ<br/>ให้ AI ช่วยกรอกข้อมูล</div>):files.map(f=>(<FileCard key={f.id} f={f} onRemove={()=>setFiles(p=>p.filter(x=>x.id!==f.id))} onAssign={code=>assignFile(f.id,code)}/>))}
          </div>
          {files.length>0&&(
            <div style={{padding:"10px 16px",borderTop:`1px solid ${T.border}`,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,background:T.bg}}>
              <div style={{textAlign:"center",padding:"6px 0",borderRadius:8,background:"#d1fae5"}}><div style={{fontSize:20,fontWeight:800,color:"#065f46"}}>{files.filter(f=>f.assignedCode).length}</div><div style={{fontSize:10,color:"#6ee7b7",fontWeight:600}}>จับคู่แล้ว</div></div>
              <div style={{textAlign:"center",padding:"6px 0",borderRadius:8,background:"#fef3c7"}}><div style={{fontSize:20,fontWeight:800,color:"#92400e"}}>{files.filter(f=>!f.assignedCode).length}</div><div style={{fontSize:10,color:"#fcd34d",fontWeight:600}}>รอกำหนดรหัส</div></div>
            </div>
          )}
          <div style={{padding:"10px 14px",borderTop:`1px solid ${T.border}`,background:"#eff6ff",fontSize:10,color:"#1e40af",lineHeight:1.6}}>
        ...
        <main style={{padding:"20px 24px",overflowY:"auto"}}>
          ...
          <div style={{textAlign:"center",padding:"16px 0",color:T.textMuted,fontSize:11,borderTop:`1px solid ${T.border}`,marginTop:8}}>
